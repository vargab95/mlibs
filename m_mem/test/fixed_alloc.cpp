...
TEST(m_fixed_allocator_tests, single_allocation)
{
    ...
    m_com_sized_data_t *data = m_fixed_allocator.sized_malloc(context, page_size / 2);
...
TEST(m_fixed_allocator_tests, exhaust_context)
{
    ...
    m_com_sized_data_t *data = m_fixed_allocator.sized_malloc(context, page_size + 1);
...
TEST(m_fixed_allocator_tests, write_to_end)
{
    ...
    m_com_sized_data_t *data = m_fixed_allocator.sized_malloc(context, sizeof(int));
